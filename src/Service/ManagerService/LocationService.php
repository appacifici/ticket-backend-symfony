...
class LocationService extends ControlService {
    ...
    public function update( int|null $id, object $input = null ) :object {
        ...
        if( !empty( $this->input->name )){      $location->setName( $this->input->name ); }
        if( !empty( $this->input->city )){      $location->setCity( $this->input->city ); }
        if( !empty( $this->input->date )){      $location->setDate( new DateTimeImmutable( $this->input->date ) ); }                  
