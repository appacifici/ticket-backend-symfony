...
************************************************************************************************************/
	...
	function JSDragDropTree()
	{
		...
		this.imageFolder = '/templatemanager/img/';
		this.folderImage = 'templatemanager/img/doc_16.png';
	...
	JSDragDropTree.prototype = {
		...
		*/
		...
		,getNodeOrders : function(initObj,saveString)
		{
			...
			//alert("tot quanti li ---------"+lis.length);
				...
				while(li){
					if(li.id){
						...
						if(saveString.length>0) {
                            initialString = saveString;
                            saveString = saveString + ',';
                        }

						var numericID = li.id.replace(/[^\|a-zA-Z0-9\_]/gi,'');
						...
						if(numericID!=''){
                            ...
                            /**
                             * TRICK PER PASSAGGI VARIABILI AGGIUNGIVE DA METTERE QUI
                             */
                            
                            //Aggiunge ai parametri il nome della varibile smarty a cui assegnare il tpl
                            var varTpl = 'varTpl='+li.getElementsByTagName('a')[0].getAttribute( "data-varTpl" );
                            var varAttrAjax = 'varAttrAjax='+li.getElementsByTagName('a')[0].getAttribute( "data-varAttrAjax" );
                            var varCores = 'cores='+li.getElementsByTagName('a')[0].getAttribute( "data-cores" );
                            var varAjax = 'ajax='+li.getElementsByTagName('a')[0].getAttribute( "data-ajax" );
                            var varCategoryNews = 'categoryNews='+li.getElementsByTagName('a')[0].getAttribute( "data-categoryNews" );
                            var varSubcategory = 'subcategory='+li.getElementsByTagName('a')[0].getAttribute( "data-subcategory" );
                            var varTypology = 'typology='+li.getElementsByTagName('a')[0].getAttribute( "data-typology" );
                            var varLimitNews = 'limitNews='+li.getElementsByTagName('a')[0].getAttribute( "data-limitNews" );
                            var varAffiliation = 'affiliation='+li.getElementsByTagName('a')[0].getAttribute( "data-affiliation" );
                            var varTrademark = 'trademark='+li.getElementsByTagName('a')[0].getAttribute( "data-trademark" );
                            
                            saveString = saveString +"|"+varAttrAjax+"|"+varTpl+"|"+varCores+"|"+varAjax+"|"+varCategoryNews+"|"+varSubcategory+"|"+varTypology+"|"+varLimitNews+"|"+varAffiliation+"|"+varTrademark;
                            console.info(saveString);
                            
							saveString = saveString + ' - ';                            
							if(li.parentNode.id!=this.idOfTree)
                                saveString = saveString + numericParentID; else saveString = saveString + '0';                                                                                                                                           
						}
		...
		{
			var message = "Clicca 'OK' per eliminare il figlio '"+ obj2.innerHTML+"' dall'alberatura!\n\nRicordati di salvare poi le modifiche cojoneeeee!";
		...
		addItem : function(nameT)
		{
			var msg2 = confirm("Per aggiungere questo nuovo modulo saranno salvate tutte le modifiche alla struttura fino a questo momento .. confermi?");
			if(msg2 == true){
				var elemento = document.getElementById("tree_ul_0");
				var new_element = document.createElement('li');
				var nomeCorretto = nameT.split("|");
				new_element.innerHTML = "<a href='#'>"+nomeCorretto[0]+"</a>";
				new_element.id = nameT;
		...
		initTree : function()
		{
			...
			for(var no=0;no<menuItems.length;no++){
				...
				if(menuItems[no].className){
					...
					folderImg.src =  "/"+this.folderImage;
				}
