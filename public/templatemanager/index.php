...
if (!empty($_POST["actionForm"]) && $_POST["actionForm"] == "getSelectList") {
    ...
    foreach ($xml as $nodo) {
        ...
        die();
} elseif (!empty($_POST["actionForm"]) && $_POST["actionForm"] == "addTpl") {
    $val = '.tpl';
...
} elseif (!empty($_POST["actionForm"]) && $_POST["actionForm"]  == "addProject") {
    $val = '.xml';
...
} elseif (empty($_SESSION["nomeFileStruttura"])) {
...
function sfogliaCartelle($dir, $tipo, $isGestione, $templateAttuale, $siteObjectsElements)
{
    ...
    foreach ($fileRoot as $myFile) {
        ...
        } elseif ($tipo == 2 && $myDir == "box" && $myFile != 'base' && $myFile != 'index') {
            $myArray[] = $myFile;
        }
    }
...
function includeFolder($folder, $directory = false)
{
    ...
    foreach ($dir as $f) {
        ...
        if (is_dir($folder . "/" . $f) && $directory == true) {
            $arrayfile[] = $f;
        } elseif (!is_dir($folder . "/" . $f) && $directory == false) {
            $arrayfile[] = $f;
        }
